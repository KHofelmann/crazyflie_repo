...
class OptiTrackSubscriber(Node):
    def __init__(self):
        ...
        self.pub_threshold_met = self.create_publisher(Bool, '/threshold_met_cf1', 10)
        self.sub_threshold_met = self.create_subscription(Bool, '/threshold_met_cf2', self.cf2_threshold_met_callback, 10)  
        self.threshold_met = False
        self.cf2_threshold_met = False
        ...
        self.target_positions = [[1.0, 0.5, 1.0],[1.0, 0.5, 2.0]] #set single position (x,y,z)
    ...
    def listener_callback(self, msg):
        ...
        if msg.header.frame_id == "world":
            ...
            #     # print(f"cf1:Reached target position {self.target_position}")
            ...
            #         self.target_position = self.target_positions[self.current_target_index]
            #         self.get_logger().info(f"cf1:Moving to next target position {self.target_position}")
            #         # print(f"cf1:Moving to next target position {self.target_position}")
            #     else:
            #         self.get_logger().info("All target positions reached. Hovering.")
            #         # print("cf1:All target positions reached. Hovering.")
            ...
            if self.is_within_threshold(self.position, self.target_position): #if drone is at desired position
                if not self.startTimer: #if the timer for hovering has not started, start it
                    ...
                    if not self.threshold_met: #if the threshold has not been met, set it to true
                        self.threshold_met = True
                        self.publish_threshold_met()
                        print("cf1: Threshold met")
                    if self.threshold_met and self.cf2_threshold_met: #if both drones have met the threshold, move to the next position
                        self.current_target_index += 1
                        if self.current_target_index < len(self.target_positions):  #if these is another target position, move to it
                            self.target_position = self.target_positions[self.current_target_index]
                            self.get_logger().info(f"cf1:Moving to next target position {self.target_position}")
                            print(f"cf1: moving to next position: {self.target_position}")
                    else:
                        print("Waiting for cf2 to reach threshold.")
                    # else:
                        # self.get_logger().info("All target positions reached. Hovering.")
    ...
    #callback function that gets the threshold_met value from cf2
    def cf2_threshold_met_callback(self, msg):
        self.cf2_threshold_met = msg.data #set the boolean to the value recieved from cf2

    #publishes the threshold_met value to communicate with cf2
